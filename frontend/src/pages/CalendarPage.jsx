    ...
    <div>
      ...
      <div className="page-header">
        ...
        <div className="toolbar">
          ...
          <button className="btn btn-icon btn-primary btn-sm" onClick={() => openCreate(toDateStr(new Date()))} title={t('common.create')}>
            <Plus size={16} />
          </button>
        </div>
      </div>
              ...
              <div className="calendar-grid">
                ...
                {calendarDays.map((d, i) => (
                  <div key={i}
                    ...
                    onDragOver={d ? (e) => handleDragOver(e, d) : undefined}
                    onDragLeave={d ? () => handleDragLeave() : undefined}
                    onDrop={d ? () => handleDrop(d) : undefined}
                    onDoubleClick={d ? () => openCreate(`${year}-${String(month).padStart(2, '0')}-${String(d).padStart(2, '0')}`) : undefined}
                    style={d ? { cursor: 'pointer' } : {}}>
                    {d && <div className="day-number">{d}</div>}
                    {d && getEventsForDay(d).map((ev) => (
                      <div key={ev.id} className="calendar-event" style={{ background: ev.color || '#3b82f6', cursor: 'grab' }}
                        title={`${ev.title}\n${ev.start_time || ''} - ${ev.end_time || ''}\n${t('common.status')}: ${ev.status || 'PENDIENTE'}`}
                        draggable
                ...
                {weekDays.map((d, i) => {
                  ...
                  return (
                    <div key={i}
                      ...
                      onDragOver={(e) => { e.preventDefault(); setDropTarget(d.getDate()); }}
                      ...
                      {dayEvents.slice(0, 4).map((ev) => (
                        <div key={ev.id} className="calendar-event" style={{ background: ev.color || '#3b82f6', cursor: 'grab', fontSize: '0.65rem', padding: '1px 2px' }}
                          title={`${ev.title}\n${ev.start_time || ''} - ${ev.end_time || ''}\n${t('common.status')}: ${ev.status || 'PENDIENTE'}`}
                          draggable
                          onDragStart={(e) => handleDragStart(ev, e)}
                          onClick={(e) => { e.stopPropagation(); openEdit(ev); }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', position: 'relative' }}>
                ...
                <div className="day-events-container" style={{ flex: 1, position: 'relative', height: `${CONTAINER_HEIGHT}px`, borderLeft: '2px solid var(--border)' }}
                  ...
                  {getEventsForDateObj(viewDate).map((ev) => {
                    ...
                    return (
                      <div key={ev.id} className="calendar-event" style={{ position: 'absolute', top: `${topPct}%`, left: '2px', right: '2px', height: `${heightPct}%`, background: ev.color || '#3b82f6', cursor: 'pointer', fontSize: '0.7rem', overflow: 'hidden', borderRadius: '4px', zIndex: resizingId === ev.id ? 10 : 1, minHeight: '20px' }}
                        onClick={() => openEdit(ev)}
                        title={`${ev.title}\n${ev.start_time || ''} - ${ev.end_time || ''}\n${t('common.status')}: ${ev.status || 'PENDIENTE'}\n${ev.observation || ''}`}>
                        ...
                        <div onMouseDown={(e) => handleResizeStart(ev, e)}
      ...
      {modalOpen && (
        <div className="modal-overlay" onClick={closeModal} role="dialog" aria-modal="true" aria-label={editingId ? t('page.calendar.edit_event') : t('page.calendar.new_event')}>
          <div ref={modalRef} className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxHeight: '90vh', overflowY: 'auto' }}>
            <h2>{editingId ? t('page.calendar.edit_title') : t('page.schedule.create_title')}</h2>
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                <label>{t('page.schedule.activity')}</label>
                <div style={{ display: 'flex', gap: '0.25rem', alignItems: 'center' }}>
                  <select value={form.activity} onChange={(e) => setForm({...form, activity: e.target.value})} required style={{ flex: 1 }} className={formErrors.activity ? 'input-error' : ''}>
                    <option value="">{t('form.select')}</option>
                    {activities.map(a => <option key={a.id} value={a.id}>{a.description}</option>)}
                  </select>
                  {formErrors.activity && <span className="field-error">{formErrors.activity}</span>}
                  <button type="button" className="btn btn-icon btn-sm btn-secondary" onClick={() => setShowNewActivity(true)} title="Nueva actividad personal">
                    <Plus size={14} />
                  </button>
                </div>
                {showNewActivity && (
            ...
            {editingId && <CommentsSection endpoint="schedule/comments" filterKey="schedule_period" filterValue={editingId} />}
